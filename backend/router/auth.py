...
import schemas 
import user_auth.utils as utils
from user_auth.oauth2 import AuthJWT
...
def login(payload: schemas.LoginUserSchema, response: Response, Authorize: AuthJWT = Depends()):
    ...
    access_token = Authorize.create_access_token(
        subject=str(user["id"]), expires_time=timedelta(minutes=ACCESS_TOKEN_EXPIRES_IN), user_claims={'role':user["role"], 'identification_number':user["identification_number"]})
